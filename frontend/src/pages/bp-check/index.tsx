...
import { bpCheckApi } from '@shared/api/endpoints';
...
export const BPCheckPage = () => {
  ...
  const checkMutation = useMutation({
    mutationFn: (data: {
      ...
      heartRate?: number;
      feelingNote?: string;
    }) => bpCheckApi.check(data),
    onSuccess: (response) => {
      const result = response.data.data;
      if (result) {
        const category = result.category?.category || '';
        let variant: 'success' | 'warning' | 'error' | 'info' = 'success';
        if (category === 'Hypertension Stage 2' || category === 'Hypertensive Crisis') {
          variant = 'error';
        } else if (category === 'Hypertension Stage 1' || category === 'Elevated') {
          variant = 'warning';
        }
        ...
        setModalData({
          ...
          details: [
            { label: 'Systolic', value: `${result.reading.systolic} mmHg` },
            { label: 'Diastolic', value: `${result.reading.diastolic} mmHg` },
            { label: 'Category', value: result.category?.category || 'Normal' },
            { label: 'Recommendation', value: result.context.recommendation || '' },
          ].filter(d => d.value),
          variant,
        });
        setModalOpen(true);
    ...
    onError: (err: any) => {
      setModalData({
        title: 'Error',
        message: err.response?.data?.error?.message || err.message || 'Failed to check BP. Please try again.',
        details: [],
        variant: 'error',
      });
      setModalOpen(true);
  ...
  const handleSubmit = () => {
    if (!hrv) {
      setError('No HRV data captured. Please take a reading first.');
      return;
    }
    if (!user) {
      ...
      <div className="max-w-2xl mx-auto">
        ...
        >
          <div className="inline-flex items-center justify-center w-16 h-16 bg-red-100 rounded-2xl mb-6">
            <Icons.Heart className="w-8 h-8 text-red-600" />
          </div>
          <h1 className="text-4xl font-bold text-gray-900 mb-3">BP Check</h1>
            ...
            {isCapturing ? (
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <ProgressBar progress={progress} color="red" showLabel />
                ...
                <div className="text-center">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-4 border-2 border-gray-300 rounded-xl focus:ring-4 focus:ring-blue-300 focus:border-blue-600 outline-none transition-all resize-none"
                      rows={4}
                    />
                  </div>
